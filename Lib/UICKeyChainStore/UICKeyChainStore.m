...
- (NSString*)stringForKey:(id)key error:(NSError* __autoreleasing*)error
{
    ...
    if (data) {
        ...
        NSError* e =
          [self.class conversionError:NSLocalizedString(@"failed to convert data to string", nil)];
        if (error) {
            *error = e;
        }
        return nil;
             ...
             error:(NSError* __autoreleasing*)error
{
    ...
    NSError* e =
      [self.class conversionError:NSLocalizedString(@"failed to convert string to data", nil)];
    if (error) {
        *error = e;
    }
    return NO;
...
- (NSData*)dataForKey:(NSString*)key error:(NSError* __autoreleasing*)error
{
    ...
    if (status == errSecSuccess) {
        ...
        if (data) {
            ...
            NSError* e = [self.class unexpectedError:NSLocalizedString(
                                                       @"Unexpected error has occurred.", nil)];
            if (error) {
                *error = e;
            }
            return nil;
             ...
             error:(NSError* __autoreleasing*)error
{
    if (!key) {
        NSError* e =
          [self.class argumentError:NSLocalizedString(@"the key must not to be nil", nil)];
        if (error) {
            *error = e;
        }
        return NO;
    ...
    if (status == errSecSuccess || status == errSecInteractionNotAllowed) {
        ...
        NSMutableDictionary* attributes = [self attributesWithKey:nil
                                                            value:data
        ...
        NSError* unexpectedError = nil;
        NSMutableDictionary* attributes = [self attributesWithKey:key
                                                            value:data
                                                            error:&unexpectedError];
    ...
    if (status == errSecSuccess) {
        ...
        for (NSDictionary* item in items) {
            if (itemClassObject == kSecClassGenericPassword) {
                ...
                [keys
                  addObject:@{ @"server" : item[@"service"] ?: @"", @"key" : item[@"key"] ?: @"" }];
            }
        }
        return keys.copy;
    } else if (status == errSecItemNotFound) {
    ...
    if (domain.length > 0) {
        ...
        NSError* error =
          [self.class argumentError:NSLocalizedString(
                                      @"the server property must not to be nil, should use "
                                      @"'keyChainStoreWithServer:protocolType:' initializer to "
                                      @"instantiate keychain store",
                                      nil)];
        if (completion) {
            completion(nil, nil, error);
        }
               ...
               completion:(void (^)(NSError* error))completion
{
    NSString* domain = self.server.host;
    if (domain.length > 0) {
        SecAddSharedWebCredential((__bridge CFStringRef)domain,
...
+ (void)requestSharedWebCredentialWithCompletion:(void (^)(NSArray UIC_CREDENTIAL_TYPE* credentials,
...
+ (void)requestSharedWebCredentialForDomain:(NSString*)domain
                                 ...
                                 completion:(void (^)(NSArray UIC_CREDENTIAL_TYPE* credentials,
                                                      NSError* error))completion
{
...
- (NSMutableDictionary*)query
{
    ...
    if (_authenticationPrompt) {
        if (floor(NSFoundationVersionNumber) >
            ...
            NSLog(@"%@",
                  @"Unavailable 'authenticationPrompt' attribute on iOS versions prior to 8.0.");
        }
    }
#endif
        ...
        if (floor(NSFoundationVersionNumber) > floor(iOS_7_1_or_10_9_2)) { // iOS 8+ or OS X 10.10+
            ...
            SecAccessControlRef accessControl =
              SecAccessControlCreateWithFlags(kCFAllocatorDefault,
                                              accessibilityObject,
                                              (SecAccessControlCreateFlags)_authenticationPolicy,
                                              &securityError);
            if (securityError) {
...
#if TARGET_OS_IOS
            NSLog(@"%@",
                  @"Unavailable 'UICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' "
                  @"attribute on iOS versions prior to 8.0.");
#else
            NSLog(@"%@",
                  @"Unavailable 'UICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' "
                  @"attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case UICKeyChainStoreItemClassGenericPassword: return kSecClassGenericPassword;
        case UICKeyChainStoreItemClassInternetPassword: return kSecClassInternetPassword;
        default: return nil;
    }
}
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case UICKeyChainStoreProtocolTypeFTP: return kSecAttrProtocolFTP;
        case UICKeyChainStoreProtocolTypeFTPAccount: return kSecAttrProtocolFTPAccount;
        case UICKeyChainStoreProtocolTypeHTTP: return kSecAttrProtocolHTTP;
        case UICKeyChainStoreProtocolTypeIRC: return kSecAttrProtocolIRC;
        case UICKeyChainStoreProtocolTypeNNTP: return kSecAttrProtocolNNTP;
        case UICKeyChainStoreProtocolTypePOP3: return kSecAttrProtocolPOP3;
        case UICKeyChainStoreProtocolTypeSMTP: return kSecAttrProtocolSMTP;
        case UICKeyChainStoreProtocolTypeSOCKS: return kSecAttrProtocolSOCKS;
        case UICKeyChainStoreProtocolTypeIMAP: return kSecAttrProtocolIMAP;
        case UICKeyChainStoreProtocolTypeLDAP: return kSecAttrProtocolLDAP;
        case UICKeyChainStoreProtocolTypeAppleTalk: return kSecAttrProtocolAppleTalk;
        case UICKeyChainStoreProtocolTypeAFP: return kSecAttrProtocolAFP;
        case UICKeyChainStoreProtocolTypeTelnet: return kSecAttrProtocolTelnet;
        case UICKeyChainStoreProtocolTypeSSH: return kSecAttrProtocolSSH;
        case UICKeyChainStoreProtocolTypeFTPS: return kSecAttrProtocolFTPS;
        case UICKeyChainStoreProtocolTypeHTTPS: return kSecAttrProtocolHTTPS;
        case UICKeyChainStoreProtocolTypeHTTPProxy: return kSecAttrProtocolHTTPProxy;
        case UICKeyChainStoreProtocolTypeHTTPSProxy: return kSecAttrProtocolHTTPSProxy;
        case UICKeyChainStoreProtocolTypeFTPProxy: return kSecAttrProtocolFTPProxy;
        case UICKeyChainStoreProtocolTypeSMB: return kSecAttrProtocolSMB;
        case UICKeyChainStoreProtocolTypeRTSP: return kSecAttrProtocolRTSP;
        case UICKeyChainStoreProtocolTypeRTSPProxy: return kSecAttrProtocolRTSPProxy;
        case UICKeyChainStoreProtocolTypeDAAP: return kSecAttrProtocolDAAP;
        case UICKeyChainStoreProtocolTypeEPPC: return kSecAttrProtocolEPPC;
        case UICKeyChainStoreProtocolTypeNNTPS: return kSecAttrProtocolNNTPS;
        case UICKeyChainStoreProtocolTypeLDAPS: return kSecAttrProtocolLDAPS;
        case UICKeyChainStoreProtocolTypeTelnetS: return kSecAttrProtocolTelnetS;
        case UICKeyChainStoreProtocolTypeIRCS: return kSecAttrProtocolIRCS;
        case UICKeyChainStoreProtocolTypePOP3S: return kSecAttrProtocolPOP3S;
        default: return nil;
    }
}
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case UICKeyChainStoreAuthenticationTypeNTLM: return kSecAttrAuthenticationTypeNTLM;
        case UICKeyChainStoreAuthenticationTypeMSN: return kSecAttrAuthenticationTypeMSN;
        case UICKeyChainStoreAuthenticationTypeDPA: return kSecAttrAuthenticationTypeDPA;
        case UICKeyChainStoreAuthenticationTypeRPA: return kSecAttrAuthenticationTypeRPA;
        ...
        case UICKeyChainStoreAuthenticationTypeHTMLForm: return kSecAttrAuthenticationTypeHTMLForm;
        case UICKeyChainStoreAuthenticationTypeDefault: return kSecAttrAuthenticationTypeDefault;
        default: return nil;
    }
}
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        ...
        case UICKeyChainStoreAccessibilityAlways: return kSecAttrAccessibleAlways;
