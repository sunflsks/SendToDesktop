...
- (NSString*)stringForKey:(id)key error:(NSError* __autoreleasing*)error {
    ...
    if(data) {
        ...
        NSError* e = [self.class conversionError:NSLocalizedString(
                                                     @"failed to convert data to string", nil)];
        if(error) {
            *error = e;
        }
        return nil;
...
- (void)sharedPasswordWithCompletion:(void (^)(NSString* account, NSString* password,
